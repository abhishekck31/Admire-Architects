...
export default function ProjectsShowcaseClient({ projects }: { projects: Project[] }) {
  ...
  // Find initial project with image. Undefined when nothing is published.
  const initialDefaultProject = projects.find(p => p.category === CATEGORIES[0] && p.image) || projects[0];

  const [activeProject, setActiveProject] = useState<Project | undefined>(initialDefaultProject);
  const [expandedId, setExpandedId] = useState<string | null>(initialDefaultProject?.id ?? null);
  ...
  const handleCategoryChange = (category: string) => {
    ...
    const firstProject = projects.find(p => p.category === category && p.image) || projects.find(p => p.category === category);
    if (firstProject) {
  ...
  if (!activeProject) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-background text-foreground px-6">
        <p className="text-[10px] uppercase tracking-[0.3em] text-brand-blue-light font-semibold mb-4">
          Projects
        </p>
        <h1 className="text-3xl md:text-5xl font-serif font-light tracking-tight text-center">
          Our showcase is being updated
        </h1>
        <p className="text-muted-foreground font-light mt-4 text-center max-w-md">
